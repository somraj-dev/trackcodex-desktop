...
const PostJobModal: React.FC<PostJobModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 bg-black/90 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="bg-[#0A0A0A] border border-primary/30 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col">
        <div className="p-6 border-b border-[#1A1A1A] bg-primary/5">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="space-y-4">
            <input
              ...
              className="w-full bg-[#000000] border border-[#1A1A1A] rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-primary outline-none"
            />
            <textarea
              required
              ...
              className="w-full bg-[#000000] border border-[#1A1A1A] rounded-xl px-4 py-3 text-white h-32 resize-none outline-none"
            />
            <div className="grid grid-cols-2 gap-4">
              <input
                ...
                className="bg-[#000000] border border-[#1A1A1A] rounded-xl px-4 py-3 text-white outline-none"
              />
              <select
                value={formData.type}
                onChange={e => setFormData({ ...formData, type: e.target.value as any })}
                className="bg-[#000000] border border-[#1A1A1A] rounded-xl px-4 py-3 text-white outline-none appearance-none"
              >
