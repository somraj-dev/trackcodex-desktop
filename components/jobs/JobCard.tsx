...
const JobCard: React.FC<JobCardProps> = ({ job, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      className="group bg-[#11141A] border border-[#1E232E] rounded-2xl p-6 hover:border-primary/50 transition-all duration-300 shadow-sm hover:shadow-xl cursor-pointer flex flex-col relative overflow-hidden"
    >
      ...
      <div className="flex items-center justify-between mb-5 relative z-10">
        ...
        <div
          ...
          className="flex items-center gap-1.5 px-2 py-1 bg-[#0A0D14] border border-[#1E232E] rounded-lg text-[10px] text-slate-400 font-bold hover:text-primary hover:border-primary/50 transition-all"
      ...
      <div className="flex items-start gap-4 mb-5 relative z-10">
        <div className="size-12 rounded-xl bg-[#0A0D14] flex items-center justify-center overflow-hidden border border-[#1E232E] shrink-0 group-hover:border-primary/30 transition-colors shadow-inner">
          <img
      ...
      <div className="flex flex-wrap gap-1.5 mb-8 relative z-10">
        {(job.techStack || []).map((tag) => (
          <span
            key={tag}
            className="px-2.5 py-1 bg-[#0A0D14] rounded-lg border border-[#1E232E] text-[10px] text-slate-400 font-black uppercase tracking-tight group-hover:border-slate-600 transition-colors"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="mt-auto pt-5 border-t border-[#1E232E]/50 flex items-center justify-between relative z-10">
