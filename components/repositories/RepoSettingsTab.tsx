    ...
    <div className="flex flex-col lg:flex-row gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 relative">
      ...
      <div className="w-full lg:w-[240px] shrink-0 space-y-1">
        {[
          ...
          >
            <span
              className={`material-symbols-outlined !text-[20px] ${activeSubTab === item.id ? "text-white" : "opacity-60"}`}
            >
              {item.icon}
            </span>
            {item.label}
        ...
        {[
          ...
          <button
            key={item.id}
            onClick={() => setActiveSubTab(item.id)}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-all ${activeSubTab === item.id ? "bg-primary text-white shadow-lg" : "text-gh-text-secondary hover:bg-gh-bg-secondary hover:text-gh-text"}`}
          >
