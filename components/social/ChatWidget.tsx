...
const ChatWidget = ({ userId = 'user-1' }) => {
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end">
            {isOpen && (
                <div className="mb-4 w-80 h-96 bg-[#0A0A0A] border border-[#1A1A1A] rounded-lg shadow-2xl flex flex-col overflow-hidden animate-fade-in-up">
                    <div className="p-3 bg-[#000000] border-b border-[#1A1A1A] flex justify-between items-center">
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-[#000000]">
                        {messages.length === 0 && (
                        ...
                        {messages.map((msg, i) => (
                            <div key={i} className={`flex ${msg.senderId === userId ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[80%] rounded-lg px-3 py-2 text-xs ${msg.senderId === userId ? 'bg-blue-600 text-white' : 'bg-[#111111] text-slate-300'}`}>
                                    {msg.content}
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="p-3 bg-[#0A0A0A] border-t border-[#1A1A1A] flex gap-2">
                        <input
                            type="text"
                            value={input}
                            ...
                            className="flex-1 bg-[#000000] border border-[#1A1A1A] rounded px-3 py-1.5 text-xs text-white focus:outline-none focus:border-blue-500"
                        />
