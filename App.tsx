...
const ProtectedApp = ({ isFocusMode }: { isFocusMode: boolean }) => {
  ...
  // If in desktop and we don't have a token, show DesktopLogin instead of normal app
  if (desktopAuth.isDesktop && !desktopAuth.isAuthenticated) {
      ...
      <div className="flex flex-col h-screen overflow-hidden">
        <main className={`flex-1 flex flex-col min-h-0 overflow-y-auto ${location.pathname === "/" ? "no-scrollbar" : "custom-scrollbar"}`}>
          <Routes>
            ...
            >
              ...
              {!isAuthenticated && (
                <>
                  ...
                  <Route path="/auth/desktop-success" element={<HandshakeSuccess />} />

                  {/* Desktop (Electron): ALL unauthenticated routes go to handshake */}
                  {typeof window !== "undefined" && !!(window as unknown as { electron?: { platform?: { isDesktop: boolean } } }).electron ? (
                    <>
                      <Route path="/login" element={<DesktopLogin />} />
                      <Route path="/signup" element={<DesktopLogin />} />
            ...
            {/* Application */}
            {(isAuthenticated || (window as any).electron?.platform?.isDesktop) && (
              <>
