...
export const socialService = {
  getFeed: async (): Promise<Post[]> => {
    try {
      const response = await apiAxios.get(`${API_BASE_URL}/community/posts`, {
  ...
  createCommunity: async (data: {
    ...
    try {
      const response = await apiAxios.post(`${API_BASE_URL}/community`, data, {
        withCredentials: true,
      });
      return response.data;
    } catch (err) {
  ...
  getCommunity: async (slug: string): Promise<Community | null> => {
    try {
      const response = await apiAxios.get(`${API_BASE_URL}/community/${slug}`, {
        withCredentials: true,
      });
      return response.data;
    } catch (err) {
