...
import { strataApi } from "../../services/hiring/strata";
...
const UserProfileDropdown = ({
  ...
  const loadStratas = async () => {
    try {
      const data = await strataApi.getMyStrata();
      setStratas(data);
    } catch (e) {
      console.error("Failed to load user stratas", e);
    }
  };
  ...
  return (
    <div className="absolute top-full right-0 mt-2 w-72 bg-gh-bg-secondary border border-gh-border rounded-xl shadow-2xl z-[500] py-3 animate-in fade-in zoom-in-95 duration-200 backdrop-blur-xl bg-gh-bg-secondary/95">
      ...
      <div className="px-1 space-y-0.5">
        ...
        />
        {!stratas.some((s) => s.id === "acme") && (
          <MenuItem
            icon="rocket_launch"
            label="Try Strata"
            onClick={() => handleNavigate("/strata/acme")}
            badge="Free"
