...
const PurchaseModal: React.FC<PurchaseModalProps> = ({ isOpen, onClose, onConfirm }) => {
    ...
    return (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-[#0A0A0A]lack/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div 
                className="bg-[#1c1f26] border border-[#1E232E] w-full max-w-md rounded-2xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-300"
            ...
            >
                ...
                <div className="p-6 border-y border-[#1E232E] space-y-6">
                    <div>
                        <p className="text-xs font-bold text-gh-text-secondary mb-2">Quantity</p>
                        <div className="flex items-center justify-between p-3 bg-[#11141A] border border-[#1E232E] rounded-lg">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="size-7 bg-[#11141A] rounded text-white">-</button>
                                <span className="w-8 text-center font-mono text-white">{quantity}</span>
                                <button onClick={() => setQuantity(q => q + 1)} className="size-7 bg-[#11141A] rounded text-white">+</button>
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-4 flex justify-end gap-3 bg-[#11141A]">
