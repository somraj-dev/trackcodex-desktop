...
const OnboardingNavItem = ({ to, icon, label }: { to: string, icon: string, label: string }) => (
    <NavLink
        ...
        className={({ isActive }) =>
            `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-bold transition-all ${isActive ? 'bg-gh-bg-secondary text-white' : 'text-slate-400 hover:bg-white/5 hover:text-white'
            }`
        }
    >
...
const OnboardingLayout = () => {
    ...
    return (
        <div className="flex-1 flex bg-gh-bg font-display">
            ...
            <main className="flex-1 p-8 bg-gh-bg">
                <Outlet />
            </main>
        </div>
    );
